...
function url(p) {
  ...
  var admin = {
    ...
    'admin/productos.html': 'productos.html',
    'admin/pedidos.html': 'pedidos.html',
    'admin/reportes.html': 'reportes.html'
  };
...
return {
  ...
  menuActivo:
   ...
   Coffee Online — Utilidades del panel de administración
   Depende de app.js (CO). Sólo se carga en las páginas de /admin.
   ========================================================================= */
...
var ADMIN = (function () {
  ...
  var MENU = [
    { clave: 'resumen',   texto: 'Resumen',   href: 'index.html',     icono: 'i-dashboard' },
    { clave: 'pedidos',   texto: 'Pedidos',   href: 'pedidos.html',   icono: 'i-clipboard' },
    { clave: 'productos', texto: 'Productos', href: 'productos.html', icono: 'i-package'   },
    { clave: 'reportes',  texto: 'Reportes',  href: 'reportes.html',  icono: 'i-chart'     }
  ];

  /* Dibuja la barra lateral y la barra superior */
  function montar(activo, titulo, acciones) {
    var s = CO.sesion();

    var enlaces = MENU.map(function (m) {
      return '<a class="barra-lateral__enlace' + (m.clave === activo ? ' is-active' : '') + '" href="' + m.href + '">' +
        CO.icono(m.icono, 'icono--sm') + '<span>' + m.texto + '</span></a>';
    }).join('');

    var lateral =
      '<aside class="barra-lateral">' +
        '<a class="marca" href="index.html">' +
          '<span class="marca__simbolo">' + CO.icono('i-logo') + '</span>' +
          '<span class="marca__texto">' +
            '<span class="marca__nombre">Coffee Online</span>' +
            '<span class="marca__etiqueta">Administración</span>' +
          '</span>' +
        '</a>' +

        '<nav class="barra-lateral__seccion">' +
          '<p class="barra-lateral__titulo">Operación</p>' +
          enlaces +
        '</nav>' +

        '<nav class="barra-lateral__seccion">' +
          '<p class="barra-lateral__titulo">Atajos</p>' +
          '<a class="barra-lateral__enlace" href="../index.html#carta">' + CO.icono('i-utensils', 'icono--sm') + '<span>Ver la carta</span></a>' +
          '<a class="barra-lateral__enlace" href="../index.html">' + CO.icono('i-arrow-left', 'icono--sm') + '<span>Volver al sitio</span></a>' +
        '</nav>' +

        '<div class="barra-lateral__pie">' +
          '<button class="barra-lateral__enlace w-full" id="admin-salir" style="color:#E8A99A">' +
            CO.icono('i-logout', 'icono--sm') + '<span>Cerrar sesión</span></button>' +
        '</div>' +
      '</aside>';

    var superior =
      '<div class="panel-admin__barra-superior">' +
        '<div>' +
          '<h1 class="titulo-pagina">' + titulo + '</h1>' +
          '<p class="diminuto atenuado">' + hoyLargo() + '</p>' +
        '</div>' +
        '<div class="espaciador"></div>' +
        (acciones || '') +
        '<div class="fila gap-2" style="padding-left:12px;border-left:1px solid var(--line)">' +
          '<span class="avatar">' + CO.iniciales(s ? s.nombre : '') + '</span>' +
          '<div class="hide-sm">' +
            '<div class="pequeno" style="font-weight:600;color:var(--ink);line-height:1.2">' + CO.escapar(s ? s.nombre : '') + '</div>' +
            '<div class="diminuto atenuado">Administrador</div>' +
          '</div>' +
        '</div>' +
      '</div>';

    var slotLateral = CO.qs('#lateral');
    if (slotLateral) slotLateral.outerHTML = lateral;

    var slotSuperior = CO.qs('#superior');
    if (slotSuperior) slotSuperior.outerHTML = superior;

    var salir = CO.qs('#admin-salir');
    if (salir) salir.addEventListener('click', CO.salir);
  }
  ...
  function hoyLargo() {
    ...
    var d = new Date();
    var t = dias[d.getDay()] + ', ' + d.getDate() + ' de ' + meses[d.getMonth()] + ' de ' + d.getFullYear();
    return t.charAt(0).toUpperCase() + t.slice(1);
  }

  /* --- Cálculos sobre los pedidos --- */
  function mismoDia(iso, fecha) {
    var a = new Date(iso);
    return a.getFullYear() === fecha.getFullYear() &&
           a.getMonth() === fecha.getMonth() &&
           a.getDate() === fecha.getDate();
  }

  function deHoy() {
    var hoy = new Date();
    return CO.pedidos().filter(function (p) { return mismoDia(p.creado, hoy); });
  }

  function deAyer() {
    var ayer = new Date();
    ayer.setDate(ayer.getDate() - 1);
    return CO.pedidos().filter(function (p) { return mismoDia(p.creado, ayer); });
  }

  function ventas(lista) {
    return lista.reduce(function (s, p) { return s + (p.estado === 'anulado' ? 0 : p.total); }, 0);
  }

  /* Ventas de los últimos N días, del más antiguo al más reciente */
  function serieDiaria(dias) {
    var salida = [];
    var etiquetas = ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'];
    for (var i = dias - 1; i >= 0; i--) {
      var d = new Date();
      d.setDate(d.getDate() - i);
      var delDia = CO.pedidos().filter(function (p) { return mismoDia(p.creado, d); });
      salida.push({
        etiqueta: etiquetas[d.getDay()],
        fecha: d,
  ...
  function ranking(limite) {
    var acumulado = {};
    CO.pedidos().forEach(function (p) {
      if (p.estado === 'anulado') return;
      p.lineas.forEach(function (l) {
        if (!acumulado[l.id]) acumulado[l.id] = { id: l.id, nombre: l.nombre, unidades: 0, importe: 0 };
        acumulado[l.id].unidades += l.cantidad;
        acumulado[l.id].importe  += l.importe;
      });
    });
    return Object.keys(acumulado).map(function (k) { return acumulado[k]; })
  ...
  function porCategoria() {
    ...
    CO.pedidos().forEach(function (p) {
      if (p.estado === 'anulado') return;
      p.lineas.forEach(function (l) {
        var cat = l.cat || (CO.producto(l.id) || {}).cat || 'otros';
        if (!acumulado[cat]) acumulado[cat] = { cat: cat, unidades: 0, importe: 0 };
        acumulado[cat].unidades += l.cantidad;
        acumulado[cat].importe  += l.importe;
      });
    });
    return Object.keys(acumulado).map(function (k) { return acumulado[k]; })
  ...
  function pintarVariacion(pct) {
    if (pct === null || !isFinite(pct)) return '<p class="estadistica__cambio atenuado">Sin comparativo</p>';
    var sube = pct >= 0;
    return '<p class="estadistica__cambio estadistica__cambio--' + (sube ? 'sube' : 'baja') + '">' +
      CO.icono(sube ? 'i-trend-up' : 'i-trend-down', 'icono--sm') +
      (sube ? '+' : '') + pct.toFixed(1) + '% vs. ayer</p>';
  }

  return {
    montar: montar, hoyLargo: hoyLargo,
    deHoy: deHoy, deAyer: deAyer, ventas: ventas,
    serieDiaria: serieDiaria, ranking: ranking, porCategoria: porCategoria,
    variacion: variacion, pintarVariacion: pintarVariacion
  };
})();
